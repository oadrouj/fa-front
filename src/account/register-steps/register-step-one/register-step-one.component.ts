...
export class RegisterStepOneComponent extends AppComponentBase {
  ...
  async validateEmail(): Promise<boolean> {
    ...
    this.emailAddressExiste = !(await this._customAccountService
      .isEmailAddresUnique(email).toPromise<boolean>())
    ...
    if (isFormValid) {
      ...
      this._accountService
        ...
        .subscribe((result: RegisterOutput) => {
          ...
          this._customAccountService.sendConfirmationEmail(this.model.emailAddress, this.model.name, result.userId).toPromise();

          this._router.navigate(['/account/redirect'], { state: { route: '/account/reister', step: '2' } });
          return;
        });
    }
  }
